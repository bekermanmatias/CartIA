...
export class CartiaController {
  ...
  @Post('tables/archive') archiveTable(@Req() request: Request, @Body() body: { id?: string }) { requireCsrf(request); return this.cartia.archiveTable(requireUserId(request), body.id ?? '', request.session.activeLocationId); }
  ...
  @Post('settings') settings(@Req() request: Request, @Body() body: { serviceOptions?: { waiter?: boolean; bill?: boolean }; visualTheme?: { primary?: string; accent?: string; paper?: string; name?: string } }) { requireCsrf(request); return this.cartia.saveSettings(requireUserId(request), body, request.session.activeLocationId); }
  @Post('dishes/save') dish(@Req() request: Request, @Body() body: { databaseId?: string; id?: string; name?: string; detail?: string; priceCents?: number; price?: string; badge?: string; category?: string; available?: boolean }) { requireCsrf(request); return this.cartia.saveDish(requireUserId(request), body, request.session.activeLocationId); }
